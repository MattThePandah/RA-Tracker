  ...
  jobs.set(jobId, state)
    ...
    ; (async () => {
      try {
        ...
        // Trigger wheel refresh if library changed significantly
        try {
          const { refreshPool } = await import('./wheelData.js')
          await refreshPool()
        } catch (err) {
          console.warn('[Library] Failed to refresh wheel pool:', err.message)
        }
      } catch (e) {
        state.state = 'failed'
